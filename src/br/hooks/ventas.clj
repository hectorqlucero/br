(ns br.hooks.ventas
  "Business logic hooks for ventas entity.
...
(defn before-load
  ...
  (println "[INFO] Loading ventas with params:" params)
  params)
...
(defn after-load
  ...
  [rows params]
  (println "[INFO] Loaded" (count rows) "ventas record(s)")
...
(defn before-delete
  ...
  (println "[INFO] Checking if ventas can be deleted. ID:" entity-id)
  {:success true})
...
(defn after-delete
  ...
  (println "[INFO] Ventas deleted successfully. ID:" entity-id)
  {:success true})
