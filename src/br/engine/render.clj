(ns br.engine.render
  (:require
   [br.engine.config :as config]
   [br.engine.query :as query]
   [br.models.grid :as grid]
   [br.models.form :as form]
   [br.models.crud]
      ...
      (let [filename (if (and (string? field-value) (re-find #"^<img" field-value))
                       (when-let [match (re-find #"src='([^']+)'" field-value)]
                         (-> (second match)
                             ...
                             first
                             (clojure.string/replace (:path br.models.crud/config) "")))
                       field-value)]
        [:div.mb-3
         ...
         (when (and filename (not (map? filename)) (not (empty? filename)))
           [:div.mb-2
            [:img {:src (str (:path br.models.crud/config) filename "?" (random-uuid))
                   :alt filename
