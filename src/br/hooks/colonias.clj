(ns br.hooks.colonias
  "Business logic hooks for colonias entity."
...
(defn before-load
  ...
  (println "[INFO] Loading colonias with params:" params)
  params)
...
(defn after-load
  ...
  [rows params]
  (println "[INFO] Loaded" (count rows) "colonias record(s)")
...
(defn before-save
  ...
  [params]
  (println "[INFO] Saving colonias...")
  (let [nombre (:nombre params)
        municipio-id (:municipio_id params)
        existing (when (and nombre municipio-id)
                   (Query crud/db ["SELECT id FROM colonias WHERE nombre = ? AND municipio_id = ?" nombre municipio-id]))]
    (cond
      (and nombre municipio-id (seq existing) (not= (:id (first existing)) (:id params)))
      {:errors {:nombre "Ya existe una colonia con ese nombre en el municipio seleccionado"}}
...
(defn after-save
  ...
  (println "[INFO] Colonias saved successfully. ID:" entity-id)
  {:success true})
...
(defn before-delete
  ...
  (println "[INFO] Checking if colonias can be deleted. ID:" entity-id)
  {:success true})
...
(defn after-delete
  ...
  (println "[INFO] Colonias deleted successfully. ID:" entity-id)
  {:success true})
