...
2. **Configure database connection** in `src/br/db/migrator.clj`:
   ```clojure
   (def db-configs
     {:sqlite
...
```
→ Update password in `src/br/db/migrator.clj`
...
Edit `src/br/db/converter.clj` to add custom conversions:
...
To migrate specific tables only, edit `src/br/db/migrator.clj`:
...
sqlite3 db/br.db .dump > dump.sql
# Manually edit dump.sql to fix syntax
mysql -u root -p br < dump.sql
...
**"Access denied"**
- Update username/password in `src/br/db/migrator.clj`
