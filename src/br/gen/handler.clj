...
(defn- gen-controller
  [handler-name]
  (str "(ns br.handlers." handler-name ".controller\n"
       "  (:require\n"
       "   [br.handlers." handler-name ".model :as model]\n"
       "   [br.handlers." handler-name ".view :as view]\n"
       "   [br.layout :refer [application]]\n"
       "   [br.models.util :refer [get-session-id]]))\n"
       "\n"
       "(defn main\n"
       "  [request]\n"
...
(defn- gen-model
  [handler-name]
  (str "(ns br.handlers." handler-name ".model\n"
       "  (:require\n"
       "   [br.models.crud :refer [Query]]))\n"))

(defn- gen-view
  [handler-name]
  (str "(ns br.handlers." handler-name ".view)\n"))
...
(def proutes-path "src/br/routes/proutes.clj")

(defn- require-entry
  "Build the require vector string for a handler controller."
  [handler-name]
  (str "[br.handlers." handler-name ".controller :as " handler-name "]"))
...
(defn- remove-handler!
  ...
  (let [dir-name (ns-name->path handler-name)
        base-dir (str "src/br/handlers/" dir-name)]
