(ns br.hooks.citas
  "Business logic hooks for citas entity.
...
(defn before-load
  ...
  (println "[INFO] Loading citas with params:" params)
  params)
...
(defn after-load
  ...
  [rows params]
  (println "[INFO] Loaded" (count rows) "citas record(s)")
...
(defn after-save
  ...
  (println "[INFO] Citas saved successfully. ID:" entity-id)
  {:success true})
...
(defn before-delete
  ...
  (println "[INFO] Checking if citas can be deleted. ID:" entity-id)
  {:success true})
...
(defn after-delete
  ...
  (println "[INFO] Citas deleted successfully. ID:" entity-id)
  {:success true})
